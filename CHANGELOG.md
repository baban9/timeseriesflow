...
- Hatch wheel/sdist config now ships both `timeseriesflow` and `adaptiveforecast` on install and in CI
- CI uses a regular install (`pip install ".[dev]"`) so import verification matches the published wheel
...
[0.2.2]: https://github.com/baban9/timeseriesflow/releases/tag/v0.2.2
